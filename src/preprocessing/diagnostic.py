...
combined["_year"]  = combined[date_col].dt.year
combined["_month"] = combined[date_col].dt.month

# Warn if date year doesn't match folder year (placeholder dates in some files)
wrong_year = combined["_year"] != combined["_year_folder"]
if wrong_year.sum() > 0:
    print(f"\n  WARN: {wrong_year.sum()} records have date-year != folder-year")
    print("        Using folder year for these records.")
    combined.loc[wrong_year, "_year"] = combined.loc[wrong_year, "_year_folder"]
